...
DEFAULT_TLS_CERT_NAME = "server.crt"
DEFAULT_TLS_KEY_NAME = "server.key"
...
def _normalize_tls_destination(path: str | None, default_name: str) -> Path:
    if path is None or path.strip() == "":
        return (Path.cwd() / DEFAULT_KEYS_DIR / "tls" / default_name).resolve()
    return Path(path).expanduser().resolve()
...
def cmd_install_tls(args: argparse.Namespace) -> None:
    cert_source = Path(args.certificate).expanduser()
    key_source = Path(args.private_key).expanduser()

    if not args.reference_only:
        if not cert_source.exists():
            raise SystemExit(f"TLS certificate source {cert_source} does not exist.")
        if not key_source.exists():
            raise SystemExit(f"TLS private key source {key_source} does not exist.")

        cert_destination = _normalize_tls_destination(args.cert_destination, DEFAULT_TLS_CERT_NAME)
        key_destination = _normalize_tls_destination(args.key_destination, DEFAULT_TLS_KEY_NAME)

        cert_destination.parent.mkdir(parents=True, exist_ok=True)
        key_destination.parent.mkdir(parents=True, exist_ok=True)

        shutil.copy2(cert_source, cert_destination)
        shutil.copy2(key_source, key_destination)
        _restrict_permissions(cert_destination, 0o640)
        _restrict_permissions(key_destination, 0o600)

        cert_path = cert_destination
        key_path = key_destination
        print(f"Copied TLS certificate to {cert_destination}.")
        print(f"Copied TLS private key to {key_destination}.")
    else:
        cert_path = cert_source
        key_path = key_source
        if not cert_path.exists():
            print(
                f"Warning: TLS certificate {cert_path} does not exist; ensure the file is provisioned before startup.",
                file=sys.stderr,
            )
        if not key_path.exists():
            print(
                f"Warning: TLS private key {key_path} does not exist; ensure the file is provisioned before startup.",
                file=sys.stderr,
            )
    ...
    mutation = _prepare_settings_mutation(update)
    _apply_mutation(mutation)
    print("Stored TLS configuration in settings store.")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    install_tls = install_sub.add_parser(
        "tls",
        help="Copy TLS certificate/key material and record their paths in data/settings.json",
    )
    install_tls.set_defaults(func=cmd_install_tls)
    install_tls.add_argument("certificate", help="Path to the TLS certificate PEM file")
    install_tls.add_argument("private_key", help="Path to the TLS private key PEM file")
    install_tls.add_argument(
        "--cert-destination",
        help="Destination path for the certificate (default keys/tls/server.crt)",
    )
    install_tls.add_argument(
        "--key-destination",
        help="Destination path for the private key (default keys/tls/server.key)",
    )
    install_tls.add_argument(
        "--reference-only",
        action="store_true",
        help="Skip copying and store the provided paths directly in settings",
    )
